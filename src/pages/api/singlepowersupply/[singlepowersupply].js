...
async function run6(req, res) {
  try {
    ...
    const allPowerSupply = client
      .db("NEXT_LEVEL_ASSIGNMENT_6")
      .collection("powersupply");

    if (req.method === "GET") {
      const { singlepowersupply } = req.query;
      const query = { _id: new ObjectId(singlepowersupply) };
      const result = await allPowerSupply.findOne(query);
      res.send(result);
    }
