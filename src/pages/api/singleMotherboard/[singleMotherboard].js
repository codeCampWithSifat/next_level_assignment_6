...
async function run6(req, res) {
  try {
    ...
    const allMotherboard = client
      .db("NEXT_LEVEL_ASSIGNMENT_6")
      .collection("motherboard");

    if (req.method === "GET") {
      const { singleMotherboard } = req.query;
      const query = { _id: new ObjectId(singleMotherboard) };
      const result = await allMotherboard.findOne(query);
      res.send(result);
    }
