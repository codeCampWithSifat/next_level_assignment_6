...
async function run7(req, res) {
  try {
    ...
    const allStorageDevice = client
      .db("NEXT_LEVEL_ASSIGNMENT_6")
      .collection("storagedevice");

    if (req.method === "GET") {
      const { singlestoragedevice } = req.query;
      const query = { _id: new ObjectId(singlestoragedevice) };
      const result = await allStorageDevice.findOne(query);
      res.send(result);
    }
